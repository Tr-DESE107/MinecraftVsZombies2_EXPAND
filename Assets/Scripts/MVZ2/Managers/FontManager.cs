...
namespace MVZ2.Managers
{
    public class FontManager : MonoBehaviour
    {
        ...
        private void InitAlmanacTagIcons()
        {
            ...
            {
                if (SpriteReference.IsValid(tagMeta.iconSprite))
                    spriteList.Add(tagMeta.iconSprite);

                if (SpriteReference.IsValid(tagMeta.backgroundSprite))
            ...
            {
                if (enumMeta.values != null)
                {
                    foreach (var valueMeta in enumMeta.values)
                    {
                        if (SpriteReference.IsValid(valueMeta.iconSprite))
            ...
            {
                var id = group.Key;
                var sprites = group.Select(r => Main.GetFinalSprite(r)).OfType<Sprite>().Where(s => s).ToArray();
        ...
        [SerializeField]
        private Material backgroundIconMaterial = null!;
        #endregion
