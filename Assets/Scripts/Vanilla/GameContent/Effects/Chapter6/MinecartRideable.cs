...
{  
    ...
    {  
        ...
        {  
            ...
            if (entity.Level.IsGameRunning())
            {
                ...
                else
                {
                    ...
                    {
                        //EXPAND「立即停止」模式：目标点拉回车身并清零速度（按住移动、松开即停）。
                        //「惯性滑行」模式则什么都不做，目标点留在前方，由阻尼追随自然减速。
                        SetTargetPosition(entity, entity.Position);
        ...
        private void FollowPointer(Entity entity, Entity? parent, Vector3 targetPosition)
        {
            ...
            var velocity = distance * VELOCITY_DAMP;
            //EXPAND 限速：追随速度上限与键盘/按钮模式同源（轨道12/无轨8，随速度等级提升）。
            //鼠标模式下距离可能很大，未限速时矿车会以远超键盘模式的速度冲刺，且速度升级蓝图无效；
            //键盘模式稳态速度本就等于该上限（目标点匀速领先），故加限速不影响其正常手感。
            var baseSpeed = parent != null ? RAIL_MOVE_SPEED : FREE_MOVE_SPEED;
            var maxSpeed = baseSpeed * (1f + GetSpeedLevel(entity.Level) * SPEED_PER_LEVEL);
        ...
        // 设置为鼠标模式即生效（无铁轨的矿车也支持鼠标跟随，目标点由 ClampToLawn 限制在场地内）。
        public static bool IsEffectiveMouseMode(LevelEngine level)
        {
            return Global.Options.GetMinecartControlMode() == MinecartControlModes.MOUSE;
        }
