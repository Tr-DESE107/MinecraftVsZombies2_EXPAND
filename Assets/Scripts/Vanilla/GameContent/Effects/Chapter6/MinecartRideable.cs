...
{  
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                {  
                    ...
                    target = hasRail ? ClampToRail(entity, parent, target) : ClampToLawn(target);  
                    SetTargetPosition(entity, target);  
                }  
            }  
            // 无论有无轨道都追随目标点；无轨道时限制在地图边界内    
        ...
        private Vector3 ClampToLawn(Vector3 targetPosition)  
        {  
            targetPosition.x = Mathf.Clamp(targetPosition.x, LevelPositions.GetBorderX(false), LevelPositions.GetBorderX(true));  
            targetPosition.z = Mathf.Clamp(targetPosition.z, LevelPositions.PICKUP_BOTTOM_BORDER, LevelPositions.PICKUP_TOP_BORDER);  
            return targetPosition;  
        }  
        private void FollowPointer(Entity entity, Entity? parent, Vector3 targetPosition)  
        {  
            targetPosition = parent != null ? ClampToRail(entity, parent, targetPosition) : ClampToLawn(targetPosition);  
