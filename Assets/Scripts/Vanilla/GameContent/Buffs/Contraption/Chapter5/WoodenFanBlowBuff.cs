...
{
    [AutoBuffDefinition(VanillaBuffNames.Contraption.woodenFanBlow)]
    public class WoodenFanBlowBuff : BuffDefinition
    {
        ...
        // 减速持续帧数：非开大（单行）240 帧 = 8 秒；开大（全场）150 帧 = 5 秒  
        public const int SLOW_DURATION = 240;
        public const int EVOKED_SLOW_DURATION = 150;
        ...
        public class Aura : AuraEffectDefinition
        {
            ...
            public override void UpdateTargetBuff(AuraEffect effect, IBuffTarget target, Buff buff)
            {
                ...
                // 给被吹到的敌人施加减速：单行 8 秒(240帧)，开大 5 秒(150帧)  
                var targetEntity = buff.GetEntity();
                if (targetEntity != null)
                {
                    bool evoked = IsEvoked(sourceBuff);
                    int slowDuration = evoked ? EVOKED_SLOW_DURATION : SLOW_DURATION;
                    targetEntity.InflictSlow(slowDuration, new EntitySourceReference(sourceEntity));
                }
            }
            private Detector detector;
            private Detector evokedDetector;
