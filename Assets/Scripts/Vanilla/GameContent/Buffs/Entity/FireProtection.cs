...
{
    [AutoBuffDefinition(VanillaBuffNames.Entity.FireProtection)]
    public class FireProtection : BuffDefinition
    {
        public FireProtection(string nsp, string name) : base(nsp, name)
        {
            // 注册回调：实体即将受到伤害前触发
        ...
        private void PreEntityTakeDamageCallback(VanillaLevelCallbacks.PreTakeDamageParams param, CallbackResult result)
        {
            ...
            // 从实体获取 FireProtection buff实例
            var buff = entity.GetFirstBuff<FireProtection>();
            if (buff == null)
                return;

            // 判断伤害是否带有"火焰"效果，若是则减免伤害
        ...
        public static readonly VanillaBuffPropertyMeta<float> PROP_Protection_Level =
            new VanillaBuffPropertyMeta<float>("FIRE_Protection_Level");
    }
}
