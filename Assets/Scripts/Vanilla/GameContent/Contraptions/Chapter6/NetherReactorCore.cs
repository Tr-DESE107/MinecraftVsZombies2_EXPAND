    ...
    public class NetherReactorCore : AIEntityBehaviour
    {
        ...
        private void Work(Entity entity)
        {
            ...
            if (productionTimer.RunToExpiredAndNotNull(entity.GetProduceSpeed()))
            {
                ...
                // 新增：产出星之碎片时，把场上的黑曜石变为爆炸黑曜石  
                ConvertObsidian(entity);

                // 新增：1/2 概率产出爆炸核心蓝图（仅非传送带关卡）  
                TrySpawnExplosionCoreBlueprint(entity);
            }
        }

        // 将以下界反应核为中心的 3*3（充能范围）内的黑曜石替换为爆炸黑曜石  
        private void ConvertObsidian(Entity entity)
        {
            var grid = entity.GetGrid();
            if (grid == null)
                return;
            ...
            // 先收集，避免在遍历格子实体的同时修改实体导致集合变动  
            var toConvert = new List<Entity>();
            for (int x = -1; x <= 1; x++)
            {
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    if (g == null)
                        continue;
                    foreach (var ent in g.GetEntities())
                    {
                        if (ent == null || ent.IsDead)
                            continue;
                        // 注意：核对 VanillaContraptionID 中黑曜石的实际成员名  
                        if (ent.IsEntityOf(VanillaContraptionID.obsidian))
                            toConvert.Add(ent);
                    }
                }
            }

            foreach (var obs in toConvert)
            {
                if (obs == null || obs.IsDead)
                    continue;
                var pos = obs.Position;
                var spawnParams = obs.GetSpawnParams();
                obs.ClearTakenGrids();
                obs.Remove();
                // 注意：核对爆炸黑曜石的实际成员名  
                entity.Spawn(VanillaContraptionID.GlowingObsidian, pos, spawnParams);
            }
        }

        // 1/2 概率产出爆炸核心蓝图，仅在非传送带关卡  
        private void TrySpawnExplosionCoreBlueprint(Entity entity)
        {
            var level = entity.Level;
            if (level.IsConveyorMode())
                return;
            var rng = entity.RNG;
            if (!rng.NextPercent(50)) // 1/2 概率  
                return;

            // 注意：核对爆炸核心的实际成员名  
            var blueprintID = LogicBlueprintID.FromEntity(VanillaContraptionID.ExplosionCore);
