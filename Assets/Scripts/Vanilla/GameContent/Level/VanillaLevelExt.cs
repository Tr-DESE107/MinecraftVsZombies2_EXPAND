...
namespace MVZ2.Vanilla.Level
{
    public static partial class VanillaLevelExt
    {
        ...
        public static void CheckGameOver(this LevelEngine level)
        {
            ...
            // 友善（harmless）敌人：进家不判负，但跑到足够左边（越过 ENEMY_LEFT_BORDER）后销毁，  
            // 避免它们残留在场上被器械持续索敌。  
            // 仅针对 harmless 且对左方阵营敌对的单位，避免误删 IZombie 等攻守互换模式里的玩家方进攻单位。  
            var harmlessToRemove = level.GetEntities(EntityTypes.ENEMY)
                .Where(e => !e.IsDead
                            && e.IsHarmless()
                            && e.IsHostile(level.Option.LeftFaction)
                            && e.IsEnemyOutsideLeft())
                .ToList();
            foreach (var enemy in harmlessToRemove)
            {
                enemy.Remove();
            }
        }
